...
function extractValidationCommands(agentResult) {
    ...
    const start = lines.findIndex((line) => /^#{1,6}\s+Validation Commands\s*$/i.test(line.trim()));
    if (start === -1) {
        return { build: [], test: [], lint: [] };
    }
...
function handleAfterAgent(ctx) {
    ...
    const warnings = [];
    if (!hasTaskReport)
        warnings.push('Missing Task Report section (expected ## Task Report heading)');
    if (!hasDownstream)
        warnings.push('Missing Downstream Context section (expected ## Downstream Context heading)');
