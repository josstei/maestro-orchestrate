...
function normalizeSegment(segment) {
    ...
    while (progressed && toks.length > 1) {
        progressed = false;
        if (/^[A-Za-z_][A-Za-z0-9_]*=/.test(toks[0])) {
            toks = toks.slice(1);
            progressed = true;
            continue;
        }
        ...
        if (WRAPPERS.has(head)) {
            ...
            while (toks.length > 1 && toks[0].startsWith('-')) {
                const flag = toks[0];
                toks = toks.slice(1);
                if (vf.has(flag) && toks.length > 1 && !toks[0].startsWith('-'))
                    toks = toks.slice(1);
            }
            while (toks.length > 1 && /^[A-Za-z_][A-Za-z0-9_]*=/.test(toks[0]))
                toks = toks.slice(1);
            progressed = true;
            continue;
        }
...
function safeRealpath(p) {
    ...
    for (;;) {
        try {
            ...
            if (e.code !== 'ENOENT')
                return abs; // unknown error: use resolved path
