...
function assertNotHostReserved(resolvedName, runtimeName) {
    ...
    if (reserved && reserved.has(resolvedName)) {
        throw new Error(`Reserved ${runtimeName} host command name "${resolvedName}" conflicts with a built-in — ` +
 ...
 */
async function expandEntryPoints(runtimeName, srcDir = DEFAULT_SRC, codeSrcDir = DEFAULT_SRC) {
    const config = getRuntimeGeneration(await getRuntimeConfig(runtimeName, codeSrcDir)).entryPoint;
    if (!config)
        return [];
    const { default: registry } = await import(pathToFileURL(path.join(codeSrcDir, 'entry-points', 'registry.js')).href);
 ...
 * @returns {Array<{ outputPath: string, content: string }>}
 */
async function expandCoreCommands(runtimeName, srcDir = DEFAULT_SRC, codeSrcDir = DEFAULT_SRC) {
    const config = getRuntimeGeneration(await getRuntimeConfig(runtimeName, codeSrcDir)).coreCommand;
    if (!config)
        return [];
    const { default: registry } = await import(pathToFileURL(path.join(codeSrcDir, 'entry-points', 'core-command-registry.js')).href);
    const templatePath = path.join(srcDir, 'entry-points', 'templates', config.templateFile);
    return runTemplateExpansion({
        runtimeName,
        registry,
