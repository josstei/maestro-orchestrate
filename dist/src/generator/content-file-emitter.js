...
function renderTemplate(template, values, agents, agentNaming) {
    ...
    content = content.replace('<!-- @roster -->', renderRosterTable(agents, { agentNaming }));
...
function renderRuntimeDoc(template, runtime, agents) {
    ...
    content = content.replace('<!-- @roster -->', renderRosterTable(agents, { agentNaming: runtime.agentNaming }));
    return finalizeContent(content);
}
...
function buildContentFileOutputs(runtimes, srcDir, packageMetadata) {
    const template = fs.readFileSync(path.join(srcDir, 'platforms', 'shared', 'runtime-context-template.md'), 'utf8');
    const agents = loadAgentRegistry(srcDir);
    const outputs = [];
    for (const runtime of Object.values(runtimes)) {
        if (!runtime.contextFile)
            continue;
    ...
    if (runtimes.claude) {
        const readmeTemplate = fs.readFileSync(path.join(srcDir, 'platforms', 'claude', 'readme-template.md'), 'utf8');
        outputs.push({
