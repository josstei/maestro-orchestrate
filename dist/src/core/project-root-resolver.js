...
function extractClientRootCandidates(clientRoots) {
    ...
    for (const clientRoot of clientRoots) {
        const uri = typeof clientRoot === 'string'
            ? clientRoot
...
function resolveProjectRootForRuntime(runtimeConfig = {}, options = {}) {
    ...
    const explicitWorkspacePath = workspaceEnvName && env[workspaceEnvName] ? env[workspaceEnvName] : null;
    const explicitRoot = resolveExistingRoot(explicitWorkspacePath);
    if (explicitRoot) {
        return explicitRoot;
    }
    const clientRoot = resolveProjectRootFromCandidates(extractClientRootCandidates(options.clientRoots));
    if (clientRoot) {
        return clientRoot;
    }
...
function requireWorkspaceRoot(projectRoot, toolName) {
    ...
    throw new WorkspaceResolutionError(`${toolName} requires an initialized workspace. Call initialize_workspace(workspace_path=...) first.`, { code: 'WORKSPACE_NOT_INITIALIZED', details: { tool_name: toolName } });
}
function requireExplicitWorkspaceRoot({ workspacePath } = {}) {
    if (!workspacePath || typeof workspacePath !== 'string' || workspacePath.includes('${')) {
        throw new WorkspaceResolutionError('initialize_workspace requires an explicit workspace_path. No implicit cwd or env fallback is used.', { code: 'WORKSPACE_REQUIRED' });
    }
    const resolved = path.resolve(workspacePath);
    if (!fs.existsSync(resolved)) {
        throw new WorkspaceResolutionError(`workspace_path does not exist: ${resolved}`, { code: 'WORKSPACE_NOT_FOUND', details: { workspace_path: resolved } });
    }
    if (isExtensionCachePath(resolved)) {
        throw new WorkspaceResolutionError(`workspace_path resolves inside an extension cache directory: ${resolved}. Provide the user workspace path instead.`, { code: 'WORKSPACE_IN_EXTENSION_CACHE', details: { workspace_path: resolved } });
    }
    try {
