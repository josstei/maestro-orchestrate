...
const agentRegistryData = JSON.parse(readFileSync(new URL('../generated/agent-registry.json', import.meta.url), 'utf8'));
const KNOWN_AGENTS = Object.freeze(agentRegistryData.map((entry) => toSnakeCase(entry.name)));
const AGENT_CAPABILITIES = Object.freeze(Object.fromEntries(agentRegistryData.map((entry) => [toSnakeCase(entry.name), entry.capabilities])));
