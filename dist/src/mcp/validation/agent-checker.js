...
const CREATION_SIGNAL_PATTERNS = /\b(implement|create|build|scaffold|write|generate|set\s*up|develop)\b/i;
/**
...
function checkAgentCapabilities(phases) {
    ...
    for (const phase of phases) {
        ...
        const touchesFiles = (Array.isArray(phase.files) && phase.files.length > 0) ||
        ...
        if (touchesFiles && !canCreateFiles(normalizedAgent)) {
            violations.push({
                ...
                detail: `Phase ${phase.id}: agent '${phase.agent}' (${getAgentCapability(normalizedAgent)}) cannot deliver file-creating tasks. Use a write-capable agent (coder, data_engineer, etc.) or split into analysis + implementation phases.`,
                severity: 'error',
            });
            continue;
        }
