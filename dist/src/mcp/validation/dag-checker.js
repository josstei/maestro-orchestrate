...
function computeDepths(phases, phaseById) {
    ...
    function getDepth(id) {
        ...
        depthMap[id] = -1;
        const maxBlockerDepth = Math.max(...phase.blocked_by.map((blockedPhaseId) => {
...
function getTransitiveDependencies(phaseId, phaseById, memo = {}) {
    ...
    for (const dependencyId of phase.blocked_by) {
        ...
        for (const transitiveDependency of getTransitiveDependencies(dependencyId, phaseById, memo)) {
...
function checkRedundantDependencies(phases, phaseById) {
    ...
    for (const phase of phases) {
        ...
        for (const dependencyId of phase.blocked_by) {
            const otherDependencies = phase.blocked_by.filter((id) => id !== dependencyId);
            let redundant = false;
            for (const otherDependencyId of otherDependencies) {
                const transitiveDependencies = getTransitiveDependencies(otherDependencyId, phaseById, memo);
...
function buildParallelizationProfile(phases, phaseById) {
    ...
    const batches = Object.entries(phases.reduce((acc, phase) => {
