...
function architectureMemoryPath(projectRoot) {
    return path.join(resolveStateDirPath(projectRoot), 'knowledge', ARCHITECTURE_MEMORY_FILENAME);
}
...
function readArchitectureMemory(projectRoot) {
    return normalizeArchitectureMemoryGraph(readJsonSafe(architectureMemoryPath(projectRoot)));
}
function writeArchitectureMemory(projectRoot, graph) {
    const next = normalizeArchitectureMemoryGraph(graph);
    atomicWriteSync(architectureMemoryPath(projectRoot), `${JSON.stringify(next, null, 2)}\n`);
    return next;
}
export { ARCHITECTURE_MEMORY_CATEGORIES, ARCHITECTURE_MEMORY_SCHEMA_VERSION, emptyArchitectureMemoryGraph, readArchitectureMemory, writeArchitectureMemory, };
