...
import { assertActiveSessionMatches, createPendingPhaseProgress, extractBody, parseSessionState, readActiveSession, readActiveSessionOrNull, resolveBasePath, serializeSessionState, writeActiveSession, } from './session-state-core.js';
...
function checkpointRelativePath(sessionId, phaseId) {
    return path.join('state', 'checkpoints', sessionId, `phase-${phaseId}.md`);
}
...
function captureCheckpoint(state, projectRoot) {
    ...
    if (!fs.existsSync(filePath)) {
        writeState(relativePath, serializeSessionState(state, checkpointBody(projectRoot)), basePath);
    }
...
function readCheckpoint(basePath, sessionId, phaseId) {
    ...
    const filePath = path.join(basePath, relativePath);
    if (!fs.existsSync(filePath)) {
        throw new NotFoundError(`Checkpoint phase-${phaseId}.md not found for session '${sessionId}'`);
    }
    return {
        filePath,
...
function handleListCheckpoints(params, projectRoot) {
    ...
    const basePath = resolveBasePath(projectRoot);
    const checkpointsDir = path.join(basePath, 'state', 'checkpoints', params.session_id);
...
function handleRestoreCheckpoint(params, projectRoot) {
    ...
    if (snapshotState.session_id !== params.session_id) {
        throw new StateError(`Checkpoint session mismatch: checkpoint is '${snapshotState.session_id}', got '${params.session_id}'`);
    }
