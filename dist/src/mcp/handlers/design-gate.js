...
import { assertPlansFilename, resolveDocumentInputVariant, writePlansDocumentContent, } from './document-input.js';
...
function validateApprovedDesignDocumentShape(params) {
    const variant = resolveDocumentInputVariant(params, {
        ...
        requireMessage: 'record_design_approval requires either design_document_path or both design_document_content and design_document_filename',
    });
    if (!variant) {
        throw new ValidationError('record_design_approval requires either design_document_path or both design_document_content and design_document_filename');
    }
...
async function resolveDesignApprovalConsent(elicit, sessionId) {
    ...
    if (!elicited || elicited.action !== 'accept') {
        if (elicited && elicited.action === 'decline') {
            throw new StateError(`Design approval was declined for session "${sessionId}"; the design gate remains unapproved.`, { code: 'DESIGN_APPROVAL_DECLINED', details: { session_id: sessionId } });
        }
        return { evidence: MODEL_ATTESTED_CONSENT };
    }
...
function listApprovedGates(projectRoot) {
    ...
    for (const entry of entries) {
        if (!entry.endsWith(GATE_FILENAME))
            continue;
...
function findOrphanedApprovedGates(projectRoot, currentSessionId) {
    return listApprovedGates(projectRoot).filter((gate) => gate.session_id !== currentSessionId);
}
/**
