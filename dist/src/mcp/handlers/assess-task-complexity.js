    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                const nested = countFiles(path.join(directory, entry.name), depth + 1, depthLimit);
                count += nested.count;
...
function detectFrameworks(directory) {
    ...
    try {
        ...
        return Object.entries(FRAMEWORK_INDICATORS)
            .filter(([, indicators]) => indicators.some((indicator) => indicator in allDependencies))
            .map(([name]) => name);
    }
    catch {
        return [];
...
function handleAssessTaskComplexity(_params, projectRoot) {
    ...
    const configFiles = CONFIG_FILES.filter((file) => fs.existsSync(path.join(projectRoot, file)));
    ...
    return {
        ...
        existing_test_infrastructure: fs.existsSync(path.join(projectRoot, 'tests')) ||
            fs.existsSync(path.join(projectRoot, '__tests__')) ||
            fs.existsSync(path.join(projectRoot, 'test')),
        lines_of_code_estimate: fileCount <= 20 ? 'low' : fileCount <= 200 ? 'moderate' : 'high',
