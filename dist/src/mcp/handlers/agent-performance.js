...
function handleGetAgentPerformance(params, projectRoot) {
    ...
    const filterAgent = params && typeof params.agent === 'string' && params.agent.length > 0
        ? params.agent
        : null;
    const accumulators = {};
    for (const record of allRecords) {
        const agent = typeof record.agent === 'string' && record.agent.length > 0
            ? record.agent
            : 'unassigned';
    ...
    for (const agent of Object.keys(accumulators).sort()) {
        ...
        byAgent[agent] = {
            ...
            avg_phase_duration_ms: samples > 0 ? Math.round(acc.total_duration_ms / samples) : 0,
