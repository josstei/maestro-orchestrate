...
function materializeSessionDocument(projectRoot, documentPath, documentKind) {
    ...
    if (!fs.existsSync(absolutePath)) {
        const context = documentKind === 'design_document'
...
function resolveImplementationPlan(params, projectRoot) {
    return resolveDocumentInput(params, {
        ...
        writeContent: (filename, content) => writePlansDocumentContent(projectRoot, filename, content, 'implementation_plan_filename'),
    });
}
/**
...
function assertNoOrphanedApprovedGate(projectRoot, currentSessionId) {
    ...
    if (orphans.length === 0)
        return;
    if (hasDesignGate(projectRoot, currentSessionId))
        return;
...
function handleCreateSession(params, projectRoot) {
    ...
    if (isDesignGateBlockingCreate(projectRoot, params.session_id)) {
        throw new StateError('Design gate entered but not approved. Call record_design_approval before create_session.', { code: 'DESIGN_GATE_UNAPPROVED' });
    }
    ...
    if (fs.existsSync(sessionPath)) {
        ...
        if (existing.status === 'in_progress') {
            throw new StateError(`Active session '${existing.session_id}' already exists (status: in_progress). Archive it first with archive_session.`);
        }
    }
    const designDocumentCandidate = params.design_document ||
    ...
    const state = {
        ...
        current_phase: params.phases && params.phases.length > 0 ? params.phases[0].id : null,
...
function handleTransitionPhase(params, projectRoot) {
    ...
    const hasNextPhaseIds = Array.isArray(params.next_phase_ids) && params.next_phase_ids.length > 0;
    if (hasNextPhaseId && hasNextPhaseIds) {
        throw new ValidationError('next_phase_id and next_phase_ids are mutually exclusive');
    }
    const hasCompletedPhase = params.completed_phase_id != null;
    if (!hasCompletedPhase && !hasNextPhaseIds && !hasNextPhaseId) {
        throw new ValidationError('At least one of completed_phase_id, next_phase_id, or next_phase_ids is required');
    }
    const mutator = ({ state }) => {
        let completedPhase;
        if (hasCompletedPhase) {
            completedPhase = state.phases.find((phase) => phase.id === params.completed_phase_id);
            if (!completedPhase) {
                throw new NotFoundError(`Phase ${params.completed_phase_id} not found in session state`);
            }
        }
        let nextPhase;
        if (hasNextPhaseId) {
            ...
            if (!nextPhase) {
                throw new NotFoundError(`next_phase_id ${params.next_phase_id} does not match any phase in session state`);
            }
        }
        const phasesToStart = [];
        if (hasNextPhaseIds) {
        ...
        if (completedPhase) {
            const { filesCreated, filesModified, filesDeleted, hasFiles } = extractFileManifest(params);
            ...
            if (hasFiles && !contextProvided) {
                throw new ValidationError(`Phase ${completedPhase.id} produced files but downstream_context is empty after normalization. ${describeDownstreamContextShape()}`, {
            ...
            if (params.review_finding_count !== undefined &&
            ...
            const durationMs = phaseDurationMs(completedPhase.started, completedPhase.completed);
...
function handleArchiveSession(params, projectRoot) {
    const { state, basePath, sessionPath, content } = assertValidActiveSession(projectRoot, params.session_id);
    const pendingRec = (state.phases || []).find((phase) => phase.requires_reconciliation === true);
    if (pendingRec) {
        throw new StateError(`Phase ${pendingRec.id} requires reconciliation before archiving. Call scan_phase_changes or reconcile_phase to resolve.`, { code: 'RECONCILIATION_PENDING', details: { phase_id: pendingRec.id } });
    }
    ...
    const archivePath = path.join(basePath, 'state', 'archive', `${params.session_id}.md`);
    ...
    const documentPaths = [state.design_document, state.implementation_plan].filter(Boolean);
    for (const documentPath of documentPaths) {
        const absoluteDocumentPath = path.resolve(path.isAbsolute(documentPath)
            ? documentPath
            : path.join(projectRoot, documentPath));
        ...
        if (fs.existsSync(absoluteDocumentPath)) {
            const destinationPath = path.join(plansArchiveDir, path.basename(absoluteDocumentPath));
...
function handleUpdateSession(params, projectRoot) {
    return withValidatedSession(projectRoot, params.session_id, ({ state }) => {
        ...
        if (updatedFields.length === 0) {
            throw new ValidationError('At least one updatable field (execution_mode, execution_backend, current_batch) is required');
        }
        state.updated = new Date().toISOString();
        return {
