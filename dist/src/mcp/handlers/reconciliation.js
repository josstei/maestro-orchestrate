...
import { assertValidActiveSession, withValidatedSession, extractFileManifest, } from './session-state-core.js';
...
function shouldSkipDir(relPath, name) {
    if (DEFAULT_IGNORE_DIRS.has(name))
        return true;
    if (relPath === DOCS_MAESTRO)
        return true;
    if (isExtensionCachePath(relPath))
        return true;
    return false;
}
/**
...
function handleScanPhaseChanges(params, projectRoot) {
    if (!isValidPhaseId(params.phase_id)) {
        throw new ValidationError('phase_id must be a positive integer or a non-empty string');
    }
    ...
    if (!phase.started) {
        throw new StateError(`Phase ${params.phase_id} has not started; nothing to scan`);
    }
...
function handleReconcilePhase(params, projectRoot) {
    ...
    const { filesCreated, filesModified, filesDeleted, hasFiles: hasFilesPayload } = extractFileManifest(params);
    ...
    if (!hasFilesPayload && !hasContextPayload) {
        throw new ValidationError('reconcile_phase requires at least one of files_created, files_modified, files_deleted, or a populated downstream_context. An empty payload cannot clear requires_reconciliation.', { code: 'RECONCILIATION_EMPTY_PAYLOAD' });
    }
