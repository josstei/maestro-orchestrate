 ...
 * @returns {object}
 */
function handleGetRuntimeContext(_params, ctx = {}) {
    const resolvedRuntimeConfig = normalizeRuntimeConfig(ctx.runtimeConfig || getDefaultRuntimeConfig());
    const services = ctx.services || {};
    const getWorkspaceSuggestion = typeof services.workspaceSuggestion === 'function' ? services.workspaceSuggestion : () => null;
    const agentNames = KNOWN_AGENTS.map((name) => resolvedRuntimeConfig.agentNaming === 'kebab-case'
        ? toKebabCase(name)
        : name);
