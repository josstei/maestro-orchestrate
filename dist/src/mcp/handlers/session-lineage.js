...
import { createPendingPhaseProgress, resolveBasePath, readActiveSessionOrNull, writeActiveSession, } from './session-state-core.js';
...
function handleListLineage(params, projectRoot) {
    ...
    const current = sources.find((entry) => entry.state.session_id === params.session_id);
    if (!current) {
