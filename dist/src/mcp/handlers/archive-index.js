...
import { parseArchivedSessionState, collectAgents, mapArchivedSessionStates, } from './archive-scan.js';
/**
...
function toSummary(state, archivePath) {
    ...
    const tokenUsage = state.token_usage && typeof state.token_usage === 'object'
        ? state.token_usage
        : {};
    return {
        ...
        token_usage: {
            ...
            by_agent: tokenUsage.by_agent && typeof tokenUsage.by_agent === 'object'
                ? tokenUsage.by_agent
                : {},
        },
...
function readArchivedSessionSummaries(projectRoot) {
    ...
    summaries.sort((a, b) => (Date.parse(b.created) || 0) - (Date.parse(a.created) || 0));
    return summaries;
}
/**
...
function matchesFilters(summary, filters) {
    if (filters.outcome && summary.outcome !== filters.outcome)
        return false;
    if (filters.agent) {
        const inAgents = summary.agents.includes(filters.agent);
        const inLedger = Object.prototype.hasOwnProperty.call(summary.token_usage.by_agent, filters.agent);
        if (!inAgents && !inLedger)
            return false;
    }
...
function handleSearchArchivedSessions(params, projectRoot) {
    const filters = {
        agent: typeof params.agent === 'string' && params.agent.length > 0
            ? params.agent
            : null,
        outcome: params.outcome === 'failed' || params.outcome === 'completed'
            ? params.outcome
            : null,
        created_after: typeof params.created_after === 'string' ? params.created_after : null,
        created_before: typeof params.created_before === 'string' ? params.created_before : null,
    };
    const sessions = readArchivedSessionSummaries(projectRoot).filter((summary) => matchesFilters(summary, filters));
...
function handleGetCostInsights(params, projectRoot) {
    ...
    return {
        ...
        latency: {
            ...
            avg_phase_duration_ms: ledgerTotals.phases > 0
