...
const VALID_RUNTIMES = new Set(fs.readdirSync(ADAPTERS_DIR)
    .filter(isAdapterFile)
    .map(extractRuntime)
    .filter((runtime) => runtime != null));
const HOOK_MAP = JSON.parse(readFileSync(new URL('../../generated/hook-registry.json', import.meta.url), 'utf8'));
