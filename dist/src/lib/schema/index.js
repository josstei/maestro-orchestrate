...
function validate(schema, value, label = 'value') {
    switch (schema.kind) {
        ...
        case 'csv': {
            if (!Array.isArray(value))
                return [`${label}: expected array, got ${describeType(value)}`];
            const errors = [];
            ...
            for (const [field, fieldSchema] of Object.entries(schema.fields)) {
                if (!(field in record)) {
                    if (!optional.has(field))
                        errors.push(`${label}.${field}: missing required field`);
                    continue;
                }
...
function assertValid(schema, value, label = 'value') {
    ...
    if (errors.length > 0) {
        throw new ValidationError(`Schema validation failed for ${label}:\n  - ${errors.join('\n  - ')}`, { details: { label, errors } });
    }
}
/**
