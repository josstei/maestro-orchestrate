...
function assertSessionId(id) {
    if (typeof id !== 'string' || !SESSION_ID_PATTERN.test(id)) {
        throw new ValidationError('Invalid session_id: must match pattern [a-zA-Z0-9_-]+', { details: { value: id } });
    }
}
/**
 * @throws {ValidationError}
...
function assertAllowlisted(value, allowlist, label) {
    ...
    if (invalid.length > 0) {
        throw new ValidationError(`Unknown ${label}: ${invalid.map((v) => `"${v}"`).join(', ')}. Known identifiers: ${permitted.join(', ')}`, { details: { invalid, permitted, label } });
    }
}
/**
 * @throws {ValidationError}
