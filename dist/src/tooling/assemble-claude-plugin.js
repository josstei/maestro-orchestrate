...
import { CLAUDE_LOCAL_PLUGIN_DIR, CLAUDE_LOCAL_BUNDLE_DIR, PROMOTED_CONTENT_COPY_MAP, buildPromotedPluginManifestFiles, } from '../platforms/claude/local-plugin-layout.js';
...
function assertAssemblyInputs(root) {
    if (!fs.existsSync(path.join(root, 'claude', 'agents'))) {
        throw new Error("Generated claude/ tree not found. Run 'just generate' (or 'npm run generate') before assembling the local Claude plugin.");
    }
