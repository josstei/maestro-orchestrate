...
function assertCleanTrackedState(root) {
    ...
    if (status !== '') {
        throw new Error(`Refusing to publish dist branch snapshot: tracked working tree is dirty:\n${status}`);
    }
}
function ensureCompiledRuntime(worktreePath) {
...
function listGeneratorOutputs(worktreePath) {
    const output = execFileSync(process.execPath, [path.join(worktreePath, 'dist', 'src', 'tooling', 'generate.js'), '--list-outputs'], { cwd: worktreePath, encoding: 'utf8' });
    return output
        .split('\n')
        .map((line) => line.trim())
        .filter(Boolean);
...
function buildOrphanSnapshotCommit(worktreePath, version) {
    ...
    return git(worktreePath, ['commit-tree', treeSha, '-m', `dist: v${version} generated snapshot`], { env: { ...process.env, ...COMMIT_IDENTITY } });
}
