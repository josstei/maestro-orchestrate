...
function pickInventoryPaths(...paths) {
    for (const inventoryPath of paths) {
        if (!INVENTORY_RELEASE_PATHS.includes(inventoryPath)) {
            throw new Error(`Package surface rule references a path missing from the artifact inventory: ${inventoryPath}`);
        }
    }
    return Object.freeze(paths);
}
const ROOT = resolvePackageRoot(moduleDirname);
const FINAL_PACKAGE_BUDGETS = Object.freeze({
    id: 'final-dist-only',
    maxEntryCount: 390,
...
const PACKAGE_SURFACE_RULES = Object.freeze([
    {
        id: 'package-metadata',
        exact: pickInventoryPaths('package.json', 'README.md', 'CHANGELOG.md', 'LICENSE', 'EXAMPLES.md'),
    },
    {
        id: 'runtime-docs',
        exact: pickInventoryPaths('GEMINI.md', 'QWEN.md', 'docs/architecture.md', 'docs/cicd.md', 'docs/flow.md', 'docs/maestro-cheatsheet.md', 'docs/overview.md', 'docs/runtime-claude.md', 'docs/runtime-codex.md', 'docs/runtime-gemini.md', 'docs/runtime-qwen.md', 'docs/usage.md'),
    },
    {
        id: 'public-bin',
        exact: pickInventoryPaths('dist/src/bin/maestro-install-codex.js', 'dist/src/bin/maestro-mcp-server.js'),
    },
    {
        id: 'root-runtime-metadata',
        exact: pickInventoryPaths('.agents/plugins/marketplace.json', '.claude-plugin/marketplace.json', '.claude-plugin/plugin.json', 'gemini-extension.json', 'qwen-extension.json'),
    },
    {
...
function assertPackageBudgets(packageInfo, entries, budgets = PACKAGE_BUDGETS.final) {
    const failures = [];
...
function verifyPackageEntries(packages, options = {}) {
...
function verifyNpmPack(root = ROOT, options = {}) {
...
runAsMain(import.meta.url, 'npm pack verification', () => {
    ...
    console.log(`Verified npm pack contents: ${result.filename} ` +
