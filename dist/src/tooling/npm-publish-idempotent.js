...
function validatePublishTag(pkg, tag) {
    ...
    if (prerelease && (!tag || tag === 'latest')) {
        throw new Error(`Refusing to publish prerelease ${pkg.name}@${pkg.version} with the latest tag; use rc, preview, or nightly.`);
    }
    if (!prerelease && tag && PRERELEASE_TAGS.has(tag)) {
        throw new Error(`Refusing to publish stable ${pkg.name}@${pkg.version} with prerelease tag "${tag}".`);
    }
}
