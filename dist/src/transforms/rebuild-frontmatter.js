 ...
 */
const FIELDS = {
    name: (ctx) => ctx.name != null && ctx.name !== '' ? [`name: ${ctx.name}`] : [],
    ...
    color: (ctx) => ctx.frontmatter.color ? [`color: ${ctx.frontmatter.color}`] : [],
    tools: (ctx) => emitBlockList('tools', ctx.tools),
    temperature: (ctx) => ctx.fm.hasTemperature && ctx.frontmatter.temperature != null
        ? [`temperature: ${ctx.frontmatter.temperature}`]
        : [],
    turns: (ctx) => ctx.fm.turnsField && ctx.frontmatter.max_turns != null
        ? [`${ctx.fm.turnsField}: ${ctx.frontmatter.max_turns}`]
        : [],
    timeout: (ctx) => ctx.fm.hasTimeout && ctx.frontmatter.timeout_mins != null
...
function rebuildFrontmatterTransform(_content, runtime, options) {
    ...
    const name = runtime.agentNaming === 'snake_case'
