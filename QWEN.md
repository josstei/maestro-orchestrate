...
Parallel batches use Qwen Code's native subagent scheduler. The scheduler only parallelizes contiguous dispatch calls, so batch turns must be dispatch-only.
...
<HARD-GATE>
Dispatch every Maestro subagent using the `dispatch` descriptor returned by `get_agent`. For Qwen, this means calling `invoke_agent(agent_name: "<agent>", prompt: "...")` with the snake_case agent name from `dispatch.agent_name`.
...
1. Call `get_agent` for the assigned agent and use the returned `dispatch` descriptor. For Qwen, call `invoke_agent` with `dispatch.agent_name` and the full delegation prompt.
