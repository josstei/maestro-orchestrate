 ...
 *     input_tokens_details.cached_tokens               (cache-read bucket)
 *
 * Falling back covers older Codex CLI versions that surfaced
 * input_tokens/output_tokens directly. Nested cached-token detail objects
 * cover OpenAI-style Chat Completions and Responses usage envelopes.
 * cached_tokens is treated as a single bucket because Codex does not expose
...
function extractUsage(invocationResult) {
  ...
  const cached =
    usage.cached_tokens != null
      ? usage.cached_tokens
      : promptDetails && promptDetails.cached_tokens != null
        ? promptDetails.cached_tokens
        : inputDetails && inputDetails.cached_tokens;
  return {
