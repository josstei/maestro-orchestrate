...
Qwen tools use canonical names with Qwen-specific overrides declared in `src/platforms/qwen/runtime-config.ts`:
...
See `src/platforms/qwen/runtime-config.ts` for the authoritative values.
...
- `npm run generate` derives `qwen-extension.json` from `package.json` with the other runtime manifests so release metadata stays aligned.
- These files are generated by `npm run generate` and are untracked in git — a fresh clone must run `npm run generate` once before local runtime use (e.g. `qwen extensions link .`). Hosted installs (`qwen extensions install <url>`) resolve from the GitHub release archive, which already bundles the generated output, so no local generate step applies there.
