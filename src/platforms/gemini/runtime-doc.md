...
The manifest launches the server via `npx` against the versioned `@josstei/maestro` npm package, with `MAESTRO_RUNTIME=gemini`. The `maestro-mcp-server` bin (`bin/maestro-mcp-server.js`) respects that env var (defaulting to `codex` if absent) and requires canonical `src/mcp/maestro-server.js` directly, running the Gemini runtime against shared source in `src/`. The repo-root `mcp/maestro-server.js` adapter still ships but is no longer the launch target. Gemini declares `primary: filesystem` and `fallback: none`.
