...
Codex plugin manifests lack a plugin-root substitution variable (unlike Claude's `${CLAUDE_PLUGIN_ROOT}` or Gemini's `${extensionPath}`), so relative paths in `args` would resolve against the user's workspace rather than the plugin directory. Maestro invokes the server via `npx` using the versioned npm package for the current release. The `maestro-mcp-server` package bin resolves to compiled `dist/src/bin/maestro-mcp-server.js`; it sets `MAESTRO_RUNTIME=codex`, overwrites `MAESTRO_EXTENSION_PATH` with the package root, and delegates to `dist/src/mcp/maestro-server.js`.
...
See `src/platforms/codex/runtime-config.ts` for the authoritative values.
...
The runtime server is invoked via `npx` rather than a local wrapper file, so the plugin ships no local `mcp/` directory under `plugins/maestro/`. The bin entrypoint source lives in `src/bin/maestro-mcp-server.ts` and emits to `dist/src/bin/maestro-mcp-server.js`; `npm run generate` derives `plugins/maestro/.mcp.json` from `package.json` so the plugin manifest and MCP package spec stay on the same version.

These files are generated by `npm run generate` and are untracked in git — local marketplace testing (`codex plugin marketplace add /absolute/path/to/maestro-orchestrate`) requires running `npm run generate` once after cloning. The published Codex marketplace entry resolves plugin content from the CI-published `dist` branch via `git-subdir` (`path: ./plugins/maestro`, `ref: dist`, in `.agents/plugins/marketplace.json`), which already carries the generated `plugins/maestro/` tree, so no local generate step applies there.
...
| Runtime payload | thin entrypoint + package-root `dist/src` | thin entrypoint + package-root `dist/src` | shared root entrypoint + `qwen/` stubs + package-root `dist/src` | npx bin + package-root `dist/src` |
