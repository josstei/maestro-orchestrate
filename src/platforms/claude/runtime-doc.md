...
The manifest launches the server via `npx` against the versioned `@josstei/maestro` npm package, with `MAESTRO_RUNTIME=claude`. The `maestro-mcp-server` bin (`bin/maestro-mcp-server.js`) respects that env var (defaulting to `codex` if absent) and requires canonical `src/mcp/maestro-server.js` directly, running the Claude runtime against shared source in `src/`. The runtime-local `claude/mcp/maestro-server.js` adapter still ships but is no longer the launch target. Claude declares `primary: filesystem` and `fallback: none`; shared skills, templates, references, and agent bodies are resolved from package-root `src/`.
