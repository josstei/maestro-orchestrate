...
import type { PolicyRule } from '../core/policy-rules.js';
...
function toCommandRegex(rule: PolicyRule): string {
  if (rule.matchType === 'word') return `.*\\b${rule.pattern}\\b.*`;
  return `.*(?:${rule.pattern}).*$`;
}

function renderRegexRule(
  rule: PolicyRule,
  ...
  if (denyMessage) lines.push(`deny_message = "${escapeTomlBasicString(denyMessage)}"`);
  return lines.join('\n');
}

function renderPrefixGroup(rules: PolicyRule[]): string {
...
function renderPolicyToml({
  denyRules = DENY_RULES,
  askRules = ASK_RULES,
}: {
  denyRules?: readonly PolicyRule[];
  askRules?: readonly PolicyRule[];
} = {}): string {
  for (const rule of denyRules) {
    if (rule.matchType === 'word') throw new Error(`Unsupported deny matchType: word (${rule.pattern})`);
  }
  ...
  if (denyPrefixes.length > 0) blocks.push(renderPrefixGroup(denyPrefixes));
...
function buildPolicyTomlOutputs(): GeneratedOutput[] {
