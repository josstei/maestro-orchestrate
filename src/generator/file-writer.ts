...
import type { FileWriter, FileWriterStats, GenerationSessionOptions } from './types.js';
 ...
 */
function safeResolve(relativePath: string, rootDir: string): string {
 ...
 */
function executeDiff(absOutputPath: string, content: string, displayPath: string): void {
  ...
  try {
    ...
    console.log(err instanceof Error && 'stdout' in err ? String(err.stdout) : '');
  } finally {
 ...
 * @param {string} displayPath
 */
function executeDryRun(absOutputPath: string, content: string, displayPath: string): void {
  const exists = fs.existsSync(absOutputPath);
  const current = exists ? fs.readFileSync(absOutputPath, 'utf8') : null;

  let status: 'CREATE' | 'UNCHANGED' | 'UPDATE';
  if (!exists) {
    status = 'CREATE';
 ...
 */
function executeWrite(absOutputPath: string, content: string, stats: FileWriterStats): void {
 ...
 * @returns {FileWriter}
 */
function createFileWriter(opts: GenerationSessionOptions): FileWriter {
   ...
   */
  function getStats(): FileWriterStats {
