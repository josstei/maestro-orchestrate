...
function renderPolicyToml({ denyRules = DENY_RULES, askRules = ASK_RULES } = {}) {
  ...
  const denyPrefixes = denyRules.filter((r) => r.matchType === 'prefix');
  if (denyPrefixes.length > 0) blocks.push(renderPrefixGroup(denyPrefixes));

  for (const rule of denyRules.filter((r) => r.matchType === 'regex')) {
    blocks.push(renderRegexRule(rule, 'deny', DENY_PRIORITY, rule.reason));
  }
