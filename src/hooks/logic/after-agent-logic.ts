 ...
 */
type ValidationCommandCategory = 'build' | 'test' | 'lint';
type ValidationCommands = Record<ValidationCommandCategory, string[]>;

function extractValidationCommands(agentResult: any): ValidationCommands {
  if (typeof agentResult !== 'string' || agentResult.length === 0) {
    return { build: [], test: [], lint: [] };
  }
  const lines = agentResult.split('\n');
  const start = lines.findIndex((line: any) =>
    /^#{1,6}\s+Validation Commands\s*$/i.test(line.trim())
  );
  if (start === -1) {
    return { build: [], test: [], lint: [] };
  }
  const result: ValidationCommands = { build: [], test: [], lint: [] };
  for (let i = start + 1; i < lines.length; i += 1) {
    ...
    if (!match) continue;
    const category = match[1]?.toLowerCase() as ValidationCommandCategory;
 ...
 */
function handleAfterAgent(ctx: any) {
