...
function ensureBaseDir(dir: any) {
...
function createHookState(baseDir: any = DEFAULT_BASE_DIR) {
  function getBaseDir() {
    return baseDir;
  }
  ...
  function setActiveAgent(sessionId: any, agentName: any) {
    try {
  ...
  function getActiveAgent(sessionId: any) {
    try {
      assertSessionId(sessionId);
    } catch {
      return '';
  ...
  function clearActiveAgent(sessionId: any) {
    try {
      assertSessionId(sessionId);
    } catch {
      return;
  ...
  function ensureSessionDir(sessionId: any) {
    try {
      assertSessionId(sessionId);
    } catch {
      return false;
  ...
  function removeSessionDir(sessionId: any) {
    try {
      assertSessionId(sessionId);
    } catch {
      return false;
