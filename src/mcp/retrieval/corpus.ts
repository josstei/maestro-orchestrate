 ...
 */
function collectFiles(phases: any) {
  const files = new Set();
 ...
 */
function collectWarnings(phases: any) {
  const warnings = [];
  for (const phase of Array.isArray(phases) ? phases : []) {
    const context =
 ...
 */
function toRecord(state: any, archivePath: any) {
 ...
 */
function buildRetrievalCorpus(projectRoot: any) {
