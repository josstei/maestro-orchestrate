 ...
 * @param {string} dir
 */
function validateOwnedDirectory(stats: any, dir: any) {
 ...
 * @returns {fs.Stats}
 */
function ensurePrivateMode(dir: any, stats: any) {
...
class KnowledgeStore {
  ...
  read() {
    return readJsonLines(this.knowledgePath()).filter(
      (record: any) => record && typeof record === 'object' && !Array.isArray(record)
    );
  }
}
