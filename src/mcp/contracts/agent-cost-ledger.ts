...
type AgentBucket = TokenUsage & { phases: number; duration_ms: number };
 ...
 */
function createAgentBucket(): AgentBucket {
 ...
 */
function normalizeTokenUsage(tokenUsage: any): TokenUsage {
  const source =
 ...
 * @returns {number}
 */
function phaseDurationMs(started: any, completed: any) {
 ...
 */
function attributePhaseCost(byAgent: any, { agent, tokenUsage, durationMs }: any = {}) {
  const ledger: Record<string, AgentBucket> =
    byAgent && typeof byAgent === 'object' ? byAgent : {};
 ...
 * @returns {Record<string, object>}
 */
function mergeAgentLedgers(target: any, source: any) {
  const merged: Record<string, AgentBucket> =
 ...
 */
function summarizeLedger(byAgent: any) {
