 ...
 */
function computeDepths(phases: any, phaseById: any) {
  const depthMap: Record<string, any> = {};

  function getDepth(id: any) {
    ...
    const maxBlockerDepth = Math.max(
      ...phase.blocked_by.map((blockedPhaseId: any) => {
 ...
 */
function getTransitiveDependencies(phaseId: any, phaseById: any, memo: any = {}) {
  if (memo[phaseId]) {
    return memo[phaseId];
  }
 ...
 */
function checkCycles(phases: any, phaseById: any) {
  const violations = [];
  const visited = new Set();
  const inStack = new Set();

  function hasCycle(id: any) {
    if (inStack.has(id)) {
      return true;
    }
 ...
 */
function checkRedundantDependencies(phases: any, phaseById: any) {
  const violations = [];
 ...
 */
function buildParallelizationProfile(phases: any, phaseById: any) {
  ...
  return {
    ...
    depth_map: depths,
    batches,
    max_batch_size: Math.max(...batches.map((batch: any) => batch.phase_ids.length)),
