 ...
 */
function checkPlanShape(plan: any) {
 ...
 */
function checkPhaseCount(phases: any, taskComplexity: any) {
 ...
 */
function checkDuplicateIds(phases: any) {
  const violations = [];
 ...
 */
function checkDanglingDependencies(phases: any) {
  const violations = [];
  const allIds = new Set(phases.map((phase: any) => phase.id));
  for (const phase of phases) {
 ...
 */
function checkPhaseFieldSchema(phases: any) {
