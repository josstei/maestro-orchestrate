...
class PlanValidationContext {
  _plan: any;
  _taskComplexity: string;
  _phaseById: Map<string | number, any> | null;
  _depths: Record<string | number, number> | null;

  /**
   * @param {unknown} plan - The plan under validation.
   * @param {string} taskComplexity - Complexity classification ('simple' | 'medium' | 'complex').
   */
  constructor(plan: any, taskComplexity: any) {
    this._plan = plan;
  ...
  get phaseById() {
    if (this._phaseById === null) {
      this._phaseById = new Map(this.phases.map((phase: any) => [phase.id, phase]));
    }
