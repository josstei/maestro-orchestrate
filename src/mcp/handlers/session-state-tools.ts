...
function handleCreateSession(params: any, projectRoot: any) {
  ...
  const state = {
    ...
    phases: params.phases.map((phase: any) => ({
      id: phase.id,
      name: phase.name,
      status: 'pending',
...
function handleGetSessionStatus(_params: any, projectRoot: any) {
...
function handleTransitionPhase(params: any, projectRoot: any) {
  ...
  const mutator = ({ state }: any) => {
    ...
    if (hasCompletedPhase) {
      completedPhase = state.phases.find(
        (phase: any) => phase.id === params.completed_phase_id
      );
      if (!completedPhase) {
    ...
    let nextPhase;
    if (hasNextPhaseId) {
      nextPhase = state.phases.find((phase: any) => phase.id === params.next_phase_id);
      if (!nextPhase) {
        throw new NotFoundError(
...
function handleArchiveSession(params: any, projectRoot: any) {
...
function handleUpdateSession(params: any, projectRoot: any) {
