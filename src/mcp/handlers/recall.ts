 ...
 * @returns {string}
 */
function pluralize(count: any, noun: any) {
 ...
 * @returns {string}
 */
function buildRationale(summary: any) {
  const parts = [];
 ...
 */
function handleRecallSimilarSessions(params: any, projectRoot: any) {
  ...
  const ranked = new Bm25Provider()
    ...
    .slice(0, limit);
  const results = ranked.map((entry: any) => ({
