 ...
 * @returns {string}
 */
function defaultMemoryPackPath(projectRoot: any) {
 ...
 */
function resolveMemoryPackPath(packPath: any, projectRoot: any) {
...
function stableKey(value: any): string {
  if (Array.isArray(value)) {
    return `[${value.map((item: any) => stableKey(item)).join(',')}]`;
  }
  if (value && typeof value === 'object') {
    return `{${Object.keys(value)
      .sort()
      .map((key: any) => `${JSON.stringify(key)}:${stableKey(value[key])}`)
      .join(',')}}`;
  }
 ...
 */
function mergeProfile(current: any, imported: any) {
 ...
 * @returns {object[]}
 */
function newRecordsByValue(currentRecords: any, importedRecords: any) {
  const seen = new Set(
    (Array.isArray(currentRecords) ? currentRecords : []).map((record: any) =>
      stableKey(record)
    )
  );
  const out = [];
 ...
 */
function mergeArchitectureMemory(current: any, imported: any) {
  const source = imported && typeof imported === 'object' ? imported : {};
  const next = { ...current };
  const counts: Record<string, any> = {};
  ...
  for (const category of ARCHITECTURE_MEMORY_CATEGORIES) {
    ...
    const seen = new Set(
      entries
        .map((entry: any) => (entry && typeof entry.value === 'string' ? entry.value : null))
        .filter(Boolean)
    );
    counts[category] = 0;
 ...
 * @throws {ValidationError}
 */
function handleImportMemoryPack(params: any, projectRoot: any) {
