# Codex Runtime Contract Fixture
...
The full request payload Codex sent to the model on the first turn of a Maestro orchestration session. Captures the registered tool list, the system instruction (including Maestro extension context), and the conversation history up to the moment of dispatch.
...
If Codex changes its subagent registration shape, this fixture stops matching `runtime-config.delegation.pattern` and the test fails.
