# Claude CLI Runtime Contract Fixture

**Captured:** [TBD — fill in when fixture is captured]
**Claude CLI version:** [TBD]

## What this fixture is

The full request payload Claude CLI sent to the model on the first turn of a Maestro orchestration session. Captures the registered tool list, the system instruction (including Maestro extension context), and the conversation history up to the moment of dispatch.
...
If Claude CLI changes its subagent registration shape, this fixture stops matching `runtime-config.delegation.pattern` and the test fails.

## How to refresh

When Claude CLI is upgraded:
