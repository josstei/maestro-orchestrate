...
**Note:** `MAESTRO_STATE_DIR` is resolved through `resolve_settings` with exported env, workspace `.env`, extension `.env`, then default `docs/maestro`. Native agent model, temperature, turn, and timeout tuning come from agent frontmatter and Gemini CLI `agents.overrides`, not Maestro process flags.
